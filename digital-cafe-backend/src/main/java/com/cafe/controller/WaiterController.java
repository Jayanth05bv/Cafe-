...
public class WaiterController {
    ...
    public WaiterController(OrderService orderService, UserRepository userRepository, CafeService cafeService, PasswordEncoder passwordEncoder) {
    ...
    public ResponseEntity<List<CafeTable>> listTables(@PathVariable Long cafeId) {
    ...
    public ResponseEntity<OrderResponse> updateStatus(@PathVariable Long id, @RequestBody ChefController.StatusUpdate body) {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        var user = userRepository.findByUsernameOrEmail(username, username)
                .orElseThrow(() -> new IllegalArgumentException("User not found: " + username));
        return ResponseEntity.ok(OrderResponse.from(orderService.updateStatus(id, Order.OrderStatus.valueOf(body.status()), user)));
    }
